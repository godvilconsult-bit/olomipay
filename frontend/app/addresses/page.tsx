...
export default function AddressesPage() {
  ...
  const [form, setForm] = useState<any>({ label: 'Home', region: 'Dar es Salaam', ward: '', lat: null, lng: null });
  ...
  function gps() {
    ...
    navigator.geolocation.getCurrentPosition(
      (p) => { setForm((f: any) => ({ ...f, lat: p.coords.latitude, lng: p.coords.longitude })); toast.success(t('Location found', 'Eneo limepatikana')); },
      () => toast.error(t('Failed', 'Imeshindwa')), { enableHighAccuracy: true },
    );
  }
  async function add() {
    if (form.lat == null) return toast.error(t('Tap GPS to get your location', 'Bonyeza GPS kupata eneo'));
    setBusy(true);
    try { await addresses.create({ label: form.label, region: form.region, ward: form.ward || undefined, lat: form.lat, lng: form.lng }); setForm({ label: 'Home', region: 'Dar es Salaam', ward: '', lat: null, lng: null }); await load(); toast.success(t('Address added', 'Anwani imeongezwa')); }
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="block">
                ...
                <select value={form.region} onChange={(e) => setForm((f: any) => ({ ...f, region: e.target.value }))} className="w-full min-h-touch rounded-2xl border border-black/15 bg-white px-3 text-ink outline-none focus:border-flame">
                  {REGIONS.map((r) => <option key={r}>{r}</option>)}
                </select>
              </label>
              <Field label={t('Ward', 'Kata')} value={form.ward} onChange={(e) => setForm((f: any) => ({ ...f, ward: e.target.value }))} />
            </div>
            <button onClick={gps} className={cn('flex w-full items-center justify-center gap-2 rounded-2xl py-3 text-sm font-semibold', form.lat != null ? 'bg-leaf/15 text-leaf-dark' : 'bg-flame/10 text-flame')}>
              <Navigation size={16} /> {form.lat != null ? `${t('Location set', 'Eneo limewekwa')} (${form.lat.toFixed(3)}, ${form.lng.toFixed(3)})` : t('Use my location (GPS)', 'Tumia eneo langu (GPS)')}
            </button>
